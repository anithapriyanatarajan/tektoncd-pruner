...
func TestDoResourceCleanup(t *testing.T) {
	tests := []struct {
		...
		{
			name: "cleanup successful resources",
			resources: []metav1.Object{
				&mockResource{
					ObjectMeta: metav1.ObjectMeta{Name: "test-1", CreationTimestamp: metav1.Time{Time: time.Now().Add(-3 * time.Hour)}},
					completed:  true,
					successful: true,
				},
				&mockResource{
					ObjectMeta: metav1.ObjectMeta{Name: "test-2", CreationTimestamp: metav1.Time{Time: time.Now().Add(-2 * time.Hour)}},
					completed:  true,
					successful: true,
				},
				&mockResource{
					ObjectMeta: metav1.ObjectMeta{Name: "test-3", CreationTimestamp: metav1.Time{Time: time.Now().Add(-1 * time.Hour)}},
					completed:  true,
					successful: true,
				},
			},
